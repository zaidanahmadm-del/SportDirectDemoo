...
export default function Game() {
  ...
  // Safe texture loader (won’t crash if a file is missing)
  const loadTex = (name: string) =>
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Optional explicit WebGL availability check
        const testGL =
          document.createElement("canvas").getContext("webgl") ||
          document.createElement("canvas").getContext("experimental-webgl");
        if (!testGL) {
          setWebglError("WebGL is disabled or blocked in this browser.");
          return;
        }
        ...
        const postR = 0.075;
        const post = (w: number, h: number, d: number) =>
          new THREE.Mesh(new THREE.BoxGeometry(w, h, d), postMat);

        const left = post(postR * 2, goalHeight, postR * 2);
        left.position.set(-goalWidth / 2, goalHeight / 2, goalZ);
        const right = post(postR * 2, goalHeight, postR * 2);
        right.position.set(goalWidth / 2, goalHeight / 2, goalZ);
        const bar = post(goalWidth, postR * 2, postR * 2);
        bar.position.set(0, goalHeight, goalZ);
        const back = post(goalWidth, postR * 2, postR * 2);
        back.position.set(0, goalHeight, goalZ - goalDepth);
        const ld = post(postR * 2, postR * 2, goalDepth);
        ld.position.set(-goalWidth / 2, goalHeight, goalZ - goalDepth / 2);
        const rd = post(postR * 2, postR * 2, goalDepth);
        rd.position.set(goalWidth / 2, goalHeight, goalZ - goalDepth / 2);

        [left, right, bar, back, ld, rd].forEach((m) => {
          m.castShadow = true;
          goalGroup.add(m);
        });
        ...
        if (netTex) {
          ...
          net = new THREE.Mesh(
            new THREE.PlaneGeometry(goalWidth, goalHeight, 24, 16),
            netMat
          );
        ...
        const ball = new THREE.Mesh(
          ...
          ballSkin
            ? new THREE.MeshStandardMaterial({ map: ballSkin, roughness: 0.45 })
        ...
        // ✅ Store everything BEFORE starting the loop (fixes .raf undefined)
        sceneRef.current = {
          scene,
          camera,
          renderer,
        ...
        const animate = () => {
          ...
          ballShadow.position.set(
            sceneRef.current.ball.position.x,
            0.001,
            sceneRef.current.ball.position.z
          );
        ...
        // start loop AFTER sceneRef.current is set
        animate();
      } catch (err) {
        ...
        setWebglError(msg); // show real cause in fallback card
      }
    })();
      ...
      <section className="text-center mb-8">
        ...
        <p className="text-base md:text-lg text-sd-black/70 mt-3 mb-8 font-medium">
          ...
          <span className="text-sd-red font-bold">
            Score once to unlock your voucher.
          </span>
        </p>
      </section>
      ...
      <section className="w-full flex flex-col items-center">
        <div className="premium-card p-6 relative mx-auto w-full max-w-[680px]">
          {webglError ? (
            <div className="bg-white p-8 rounded-lg border-2 border-sd-light-border text-center">
              ...
              <h3 className="text-2xl font-heading font-black text-sd-blue mb-4">
                3D GAME UNAVAILABLE
              </h3>
        ...
        <div className="flex justify-center items-center gap-4 mt-6 flex-wrap w-full max-w-[680px]">
          <div className="premium-card px-5 py-3 text-center">
            <div className="text-xl font-heading font-black text-sd-blue">
              {attempts}
            </div>
          ...
          <div className="premium-card px-5 py-3 text-center">
            <div className="text-xl font-heading font-black text-sd-red">
              {goals}
            </div>
            <div className="text-xs text-sd-black/60 font-bold uppercase tracking-wide">
              Goals
        ...
        <div className="fixed inset-0 bg-sd-black/90 flex items-center justify-center z-50 fade-in pointer-events-auto">
          <Card className="max-w-sm mx-4 relative overflow-hidden premium-card bounce-in">
            <CardContent className="pt-8 pb-8 text-center">
              <h2 className="text-5xl font-heading font-black text-sd-red mb-6">
                GOAL!
              </h2>
              <p className="text-xl font-bold text-sd-black mb-8">
